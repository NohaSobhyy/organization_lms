...
class DashboardController extends Controller
{
    ...
    {
        // $batches = StudyClass::latest()->limit(2)->get()->pluck('id');
        $batch = StudyClass::latest()->first();
        $students = Student::whereHas('registeredUser.purchases', function ($query) use ($batch) {
            $query->where('class_id', $batch->id)->whereNotNull('bundle_id')->whereNull('form_fee');
        })->groupBy('students.id')->orderBy('id', 'DESC')->get();
        $export = new AboutUsExport($students);

        return Excel::download($export, 'عرفونا منين.xlsx');
    }
