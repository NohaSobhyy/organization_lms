    ...
    public function setPaymentAccounting($order, $type = null)
    {
        try {
            ...
            if ($order->is_charge_account) {
                ...
                // $user = $order->user;
                //     $usercharge = $user->getAccountingCharge();
                //     if ($usercharge > $order->amount) {
                //         $new_amount =  $order->amount;
                //     } else {
                //         $new_amount = $usercharge;
                //     }
                    // $new_amount = $usercharge-$order->total_amount;
                    Accounting::create([
        ...
        if (!empty($order)) {
            ...
            if (!empty($data['order']) && $data['order']->status === Order::$paid) {
                ...
                if (!empty($service_sale)) {
                    return redirect('/panel/services/requests')->with(['toast' => $toastData, 'success' => "تم ارسال الطلب بنجاح وهو قيد المراجعة من قبل الإدارة المختصة"])
