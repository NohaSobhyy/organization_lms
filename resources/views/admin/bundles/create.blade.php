                                    ...
                                    <div class="row">
                                        <div class="col-12 col-md-6">
                                            ...
                                            @else
                                                <div class="form-group mt-15">
                                                    <label
                                                        class="input-label">{{ 'البرامج المزدوجة لهذا البرنامج' }}</label>

                                                    <select id=""
                                                        class="form-control select2 @error('additions_bundle_id')  is-invalid @enderror"
                                                        name="additions_bundle_id[]" id='additions_bundle_id' required multiple
                                                        data-placeholder="{{ trans('public.choose_category') }}">
                                                        <option disabled>
                                                            {{ trans('public.choose_category') }}</option>
                                                        {{-- Loop through top-level categories --}}
                                                        @foreach ($categories as $category)
                                                            <optgroup label="{{ $category->title }}">
                                                                ...
                                                                @foreach ($category->programs as $bundleItem)
                                                                    <option value="{{ $bundleItem->id }}"
                                                                        @if (in_array($bundleItem->id, old('additions_bundle_id', [])) ||
                                                                                (!empty($bundle) && $bundle->additionBundles->contains($bundleItem->id))) selected @endif>
                                                                        {{ $bundleItem->title }}</option>
                                                                @endforeach
