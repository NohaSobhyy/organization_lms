...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive text-center">
                <table class="table table-striped font-14">
                    ...
                    @foreach ($bundlstudents as $index => $bundlstudent)
                        <tr>
                            <td>{{ ++$index }}</td>
                            <td>{{ $bundlstudent->student->registeredUser?->user_code }}</td>

                            <td class="text-left">
                                <div class="d-flex align-items-center">
                                    <div class="media-body ml-1">
                                        <div class="mt-0 mb-1 font-weight-bold">
                                            {{ $bundlstudent->student ? $bundlstudent->student->ar_name : null }}</div>

                                        @if ($bundlstudent->student->registeredUser->mobile)
                                            <div class="text-primary text-left font-600-bold" style="font-size:12px;">
                                                {{ $bundlstudent->student->registeredUser->mobile }}</div>
                                        @endif

                                        @if ($bundlstudent->student->registeredUser->email)
                                            <div class="text-primary text-small font-600-bold">{{ $bundlstudent->student->registeredUser->email }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="text-left">
                                @if (!empty($bundlstudent->student->identity_img))
                                    <a href="/store/{{ $bundlstudent->student->identity_img }}" target="_blank">
                                        <img src="/store/{{ $bundlstudent->student->identity_img }}" alt="image"
                            ...
                            <td>
                                {{ $bundlstudent->created_at ? dateTimeFormat(strtotime($bundlstudent->created_at), 'j M Y | H:i'):'----' }}
                            </td>

                            <td>
                                @if ($bundlstudent->student->registeredUser->ban and !empty($bundlstudent->student->registeredUser->ban_end_at) and $bundlstudent->student->registeredUser->ban_end_at > time())
                                    <div class="mt-0 mb-1 font-weight-bold text-danger">{{ trans('admin/main.ban') }}
                                    </div>
                                    <div class="text-small font-600-bold">Until
                                        {{ dateTimeFormat($bundlstudent->student->registeredUser->ban_end_at, 'Y/m/j') }}</div>
                                @else
                                    <div
                                        class="mt-0 mb-1 font-weight-bold {{ $bundlstudent->student->registeredUser->status == 'active' ? 'text-success' : 'text-warning' }}">
                                        {{ trans('admin/main.' . $bundlstudent->student->registeredUser->status) }}</div>
                                @endif
                            </td>
                            ...
                            <td class="text-center mb-2" width="120">
                                ...
                                @can('admin_users_impersonate')
                                    <a href="{{ getAdminPanelUrl() }}/users/{{ $bundlstudent->student->registeredUser->id }}/impersonate" target="_blank"
                                ...
                                @can('admin_users_edit')
                                    <a href="{{ getAdminPanelUrl() }}/users/{{ $bundlstudent->student->registeredUser->id }}/edit"
                                        class="btn-transparent  text-primary" data-toggle="tooltip" data-placement="top"
                                ...
                                @can('student_bundle_delete')
                                   @include('admin.includes.delete_button', [
                                       'url' => getAdminPanelUrl() . '/bundleStudents/' . $bundlstudent->id . '/delete',
                                       'btnClass' => '',
                                     'deleteConfirmMsg' => trans('admin/main.delete_confirm_msg'),
                                     'tooltip' => 'حذف الطالب من البرنامج'
                                               ])
                                   @endcan
                            </td>
        ...
        <div class="card-footer text-center">
            {{ $bundlstudents->appends(request()->input())->links() }}
        </div>
    </div>
