                            ...
                            <div class="quiz-card">
                                ...
                                @if($question->type === \App\Models\QuizzesQuestion::$descriptive)
                                    ...
                                    <div class="form-group mt-35">
                                        ...
                                        <textarea rows="10" name="question[{{ $question->id }}][correct_answer]" @if(empty($newQuizStart) or ($newQuizStart->quiz->creator_id != $authUser->id and !$newQuizStart->quiz->webinar->isPartnerTeacher($authUser->id))) disabled @endif class="form-control">{{ $question->correct }}</textarea>
                                    </div>

                                    @if(!empty($newQuizStart) and ($newQuizStart->quiz->creator_id == $authUser->id or $newQuizStart->quiz->webinar->isPartnerTeacher($authUser->id)))
                                        <div class="form-group mt-35">
