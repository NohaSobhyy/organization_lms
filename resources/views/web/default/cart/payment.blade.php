        ...
        <form action="/payments/payment-request" method="post" class=" mt-25" enctype="multipart/form-data" id="cartForm">
            ...
            @if (!empty($enableCoupon))
                <div class="custom-control custom-switch mb-3">
                    ...
                    <label class="custom-control-label" for="discount-checkbox">لدي كوبون خصم</label>
                </div>
            @endif

            @if (!empty($enableCoupon))
                {{-- substraction from charge amount --}}
                <div class="custom-control custom-switch">
                    <input type="checkbox" class="custom-control-input" id="charge-checkbox" name="sub_from_amount">
                    <label class="custom-control-label" for="charge-checkbox"> من حساب الشحن</label>
                </div>
            @endif
            ...
            <div class="row mt-30">
                ...
                {{-- charge --}}
                <div class="col-12 col-lg-6 d-none" id="chargeSection">
                    <div class="charge-account-radio">
                ...
                {{-- discount --}}
                <section class="col-12 col-lg-6 row">
                    <h3 class="section-title d-none">{{ trans('cart.coupon_code') }}</h3>
                    {{-- order section --}}
                    <div class="col-12 rounded-sm shadow pb-20 px-20">
                        ...
                        <div class="cart-checkout-item">
                            <h4 class="text-secondary font-14 font-weight-500">خصم من حساب الشحن </h4>
                            <span class="font-14 text-gray font-weight-bold">
                                <span id="totalCharge">
                                    0
                                </span>
                        ...
                        <div class="cart-checkout-item border-0">
                            <h4 class="text-secondary font-14 font-weight-500">{{ trans('cart.total') }}
                            </h4>
                    ...
                    {{-- coupon section --}}
                    <div class="col-12 rounded-sm shadow py-25 px-20 mt-30 d-none" id='discountSection'>
                        ...
                        <p id="checkCoupon" class="btn btn-sm btn-primary">{{ trans('cart.validate') }}
                        </p>
...
@push('scripts_bottom')
    ...
    <script>
        window.onload = function() {
            ...
            let chargeSection = document.getElementById('chargeSection');
            ...
            chargeCheckbox.onchange = function(e) {
        ...
        function toggleCharge() {
            ...
            let chargeCheckbox = document.getElementById('charge-checkbox');
            let total = @json($order->total_amount);
            let charge = @json($userCharge);
            if (chargeCheckbox.checked) {
                totalCharge.textContent = '-' + (total > charge ? charge : total) + 'ر.س';
            } else {
                totalCharge.textContent = 0;
            }
        }
    </script>
@endpush
